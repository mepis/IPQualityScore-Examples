...
add_filter('fluentform_validate_input_item_input_email', function ($errorMessage, $field, $formData, $fields, $form) {
    $phone_validator = new ipqsValidater;
    // #############################
    // Start Edit Configs
    // ############################

    $phone_validator->ipqs_api_key = "";
    $error_message = ["Invalid Data"];
    
    // #############################
    // End Edit Configs
    // #############################
    $phone_validator->enable_phone_validation = false;
    $phone_validator->enable_email_validation = true;
    $phone_validator->enable_ip_validation = true;
    $phone_validator->email = $formData['email'];
    $error = [$error_message];
    $did_pass = $phone_validator->validate_data();
    if (!$did_pass) {
        $errorMessage = $error;
    }
    return $errorMessage;
...
add_filter('fluentform_validate_input_item_phone', function ($errorMessage, $field, $formData, $fields, $form) {
    $email_validator = new ipqsValidater;
    // #############################
    // Start Edit Configs
    // ############################

    $email_validator->ipqs_api_key = "";
    $error_message = ["Invalid Data"];
   
    // #############################
    // End Edit Configs
    // #############################
    $email_validator->enable_phone_validation = true;
    $email_validator->enable_email_validation = false;
    $email_validator->enable_ip_validation = true;
    $email_validator->phone = $formData['phone'];
    $error = [$error_message];
    $did_pass = $email_validator->validate_data();
    if (!$did_pass) {
        $errorMessage = $error;
    }
    return $errorMessage;
